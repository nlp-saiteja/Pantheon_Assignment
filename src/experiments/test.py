...
class Test(object):
    ...
    def run_without_tunnel(self):
        ...
        cmd = ['python2.7', self.cc_src, self.run_first, port]
        ...
        sh_cmd = 'python2.7 %s %s $MAHIMAHI_BASE %s' % (
    ...
    def run_tunnel_managers(self):
        ...
        if self.mode == 'remote':
            if self.server_side == 'local':
                ts_manager_cmd = ['python2.7', self.tunnel_manager]
            else:
                ts_manager_cmd = self.r['ssh_cmd'] + [
                    'python2.7', self.r['tunnel_manager']]
        else:
            ts_manager_cmd = ['python2.7', self.tunnel_manager]
        ...
        if self.mode == 'remote':
            if self.server_side == 'local':
                ...
                tc_manager_cmd = ['python2.7', self.tunnel_manager]
        else:
            tc_manager_cmd = self.mm_cmd + ['python2.7', self.tunnel_manager]
    ...
    def run_first_side(self, tun_id, send_manager, recv_manager,
        ...
        if self.run_first == 'receiver':
            ...
            first_cmd = 'tunnel %s python2.7 %s receiver %s\n' % (
                tun_id, first_src, port)
            second_cmd = 'tunnel %s python2.7 %s sender %s %s\n' % (
            ...
            first_cmd = 'tunnel %s python2.7 %s sender %s\n' % (
                tun_id, first_src, port)
            second_cmd = 'tunnel %s python2.7 %s receiver %s %s\n' % (
...
def pkill(args):
    ...
    if args.mode == 'remote':
        ...
        cmd = r['ssh_cmd'] + ['python2.7', remote_pkill_src,
                              '--kill-dir', r['base_dir']]
        call(cmd)

    pkill_src = path.join(context.base_dir, 'tools', 'pkill.py')
    cmd = ['python2.7', pkill_src, '--kill-dir', context.src_dir]
    call(cmd)
